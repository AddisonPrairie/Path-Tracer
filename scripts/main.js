window.onload = async () => {
    ...
    //let dorsayFile = await fetch("media/musee-dorsay.obj").then(f => f.text())

    const scene = initScene(device)

    const t0 = Date.now()

    //const qwantani = await fetchHdr("media/qwantani_dusk_1_2k.hdr")
    //scene.setEnvironmentLight("hdr", qwantani)
    ...
    scene.instanceMesh(buddhaModel, [0, 0, 2.2], [Math.PI / 2, 0, 0], [7, 7, 7], whiteDiffuse)
    scene.instanceMesh(cubeModel, [-3, 3, 1.2], [0, 0, Math.PI / 4], [1.2, 1.2, 1.2], whiteDiffuse)
    scene.instanceMesh(cubeModel, [3, 1, 2], [0, 0, Math.PI / 3], [1, 1, 2], ggxSmith)
    ...
    scene.instanceMesh(floorModel, [ 0, -5,  5], [Math.PI / 2, 0, 0], [5, 5, 5], mirror)
